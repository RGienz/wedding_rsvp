    ...
    <div class="max-w-7xl mx-auto space-y-10">
      ...
      <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div class="bg-white p-4 border border-stone-200 rounded text-center">
          <p class="text-[10px] uppercase tracking-wider text-stone-400 font-medium">Total Codes</p>
        ...
        <div class="bg-white p-4 border border-stone-200 rounded text-center">
          <p class="text-[10px] uppercase tracking-wider text-rose-600 font-medium">Declined</p>
        ...
        <form @submit.prevent="addGuest" class="flex flex-col sm:flex-row gap-4 items-end">
          <div class="flex-1 w-full">
            ...
            <input 
              ...
              type="text" 
              required 
              placeholder="e.g., Professor Johnston" 
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse min-w-[1100px]">
            ...
            <tbody class="divide-y divide-stone-100 text-xs text-stone-600">
              <tr v-for="guest in guests" :key="guest.id" class="hover:bg-stone-50/60 transition-colors vertical-align-middle">
                ...
                <template v-if="editingId === guest.id && editForm">
                  ...
                  <td class="p-3 text-center">
                    <select v-model.number="editForm.guests" class="w-full bg-[#fcfbf9] border border-stone-300 px-2 py-1 text-xs rounded text-center text-stone-800">
                  ...
                  <td class="p-3">
                    <input v-model="editForm.plusOneName" type="text" placeholder="Enter companion name..." class="w-full bg-[#fcfbf9] border border-stone-300 px-2 py-1 text-xs rounded placeholder-stone-300 text-stone-800" />
                  </td>
                  
                  <td class="p-3 text-center">
                    <select v-model="editForm.shuttleRequired" class="w-full bg-[#fcfbf9] border border-stone-300 px-2 py-1 text-xs rounded text-center text-stone-800">
                ...
                <template v-else>
                  ...
                  <td class="p-4 text-center font-medium">
                    {{ guest.attending === 'yes' ? guest.guests : guest.guests }}
                  </td>
                  
                  <td class="p-4 italic text-stone-600">
                    {{ guest.plusOneName || '—' }}
                  </td>
                  
                  <td class="p-4 text-center uppercase tracking-widest text-[10px] font-bold">
                    <span v-if="guest.shuttleRequired === 'yes'" class="text-sky-600">YES</span>
                    <span v-else-if="guest.shuttleRequired === 'no'" class="text-stone-400">NO</span>
